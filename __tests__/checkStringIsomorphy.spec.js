const isomorphy = require("../src/checkStringIsomorphy");

describe("Input: ", () => {
  it("character only strings -> returns true", () => {
    let s1 = "egg";
    let s2 = "add";
    expect(isomorphy(s1, s2)).toBe(true);
  });

  it("character only strings -> returns false", () => {
    let s1 = "hello";
    let s2 = "world";
    expect(isomorphy(s1, s2)).toBe(false);
  });
  ...
  it("first string null -> throws TypeError", () => {
    let s1 = null;
    let s2 = "weather";
    expect(() => isomorphy(s1, s2).toThrow(TypeError));
  });
