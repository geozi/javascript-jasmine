const construct = require("../src/stringConstructPossibility");
...
describe("Input: ", () => {
  ...
  it("letter only strings, longer s -> returns true", () => {
    let s = "aab";
    let t = "aa";
    expect(construct(s, t)).toBe(true);
  });
  ...
  it("letter only strings, longer s  -> returns false", () => {
    let s = "hello";
    let t = "bye";
    expect(construct(s, t)).toBe(false);
  });
  ...
  it("s as a null -> throws TypeError", () => {
    let s = null;
    let t = "hello";
    expect(() => construct(s, t).toThrow(TypeError));
  });
